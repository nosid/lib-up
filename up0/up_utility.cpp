...
auto up_utility::type_display_name(const std::type_info& type_info) -> std::string
{
    ...
    if (status == 0) {
        ...
        up::raise<runtime>("type-demangle-error", type_info.name(), status);
    }
}
...
void up_utility::cformat(up::buffer& buffer, const char* format, va_list ap)
{
    ...
    va_end(aq);
    if (rv < 0) {
        up::raise<runtime>("cformat-error", std::string(format), rv);
    }
    ...
    if (size < buffer.capacity()) {
        ...
        if (rv < 0) {
            up::raise<runtime>("cformat-error", std::string(format), size, rv);
        }
        size = up::ints::cast<std::size_t>(rv);
        if (size < buffer.capacity()) {
            buffer.produce(size);
        } else {
            up::raise<runtime>("cformat-error", std::string(format), size);
        }
    }
}
...
{
    up::raise<runtime>(std::move(source), std::move(insight));
}
