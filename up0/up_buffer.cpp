...
namespace
{
    ...
    auto get_mutable_header(char* core) -> header&
    {
        if (core) {
            ...
            UP_RAISE(runtime, "buffer-null-state");
        }
    }
    ...
    auto core_reallocate(char* core, const header& h) -> char*
    {
        ...
        if (char* temp = up::char_cast<char>(std::realloc(core, core_size(h)))) {
            ...
            UP_RAISE(runtime, "buffer-out-of-memory", h);
        }
    }
...
void up_buffer::buffer::consume(size_type n)
{
    if (_core) {
        ...
        if (pos > h._cold_pos) {
            UP_RAISE(out_of_range, "buffer-consume-overflow", h, n);
        } else {
            h._warm_pos = pos;
        }
    } else if (n) {
        UP_RAISE(out_of_range, "buffer-consume-overflow", null_header, n);
    } // else: nothing
}
...
void up_buffer::buffer::produce(size_type n)
{
    if (_core) {
        ...
        if (pos > h._size) {
            UP_RAISE(out_of_range, "buffer-produce-overflow", h, n);
        } else {
            h._cold_pos = pos;
        }
    } else if (n) {
        UP_RAISE(out_of_range, "buffer-produce-overflow", null_header, n);
    } // else: nothing
}
